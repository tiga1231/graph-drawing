...
function drawThumbnail(graph, svg) {
    ...
    function updateScales() {
        ...
        svg.sx.domain(xExtent).range(xViewport);
        svg.sy.domain(yExtent).range(yViewport);
    }

    function draw() {
        svg.selectAll(".edge").data(graph.edges).exit().remove();
        let edges = svg
            .selectAll(".edge")
            .data(graph.edges)
            .enter()
        ...
        let nodes = svg
            ...
            .attr(
                "transform",
                (d) => `translate(${svg.sx(d.x)},${svg.sy(d.y)})`,
            )
            .moveToFront();
        let texts = nodes.selectAll("text").text((d) => d.id);
...
function evaluate(graph) {
    let n_neighbors = graph.graphDistance.map((row) => {
        return row.reduce((a, b) => (b == 1 ? a + 1 : a), 0);
    });
    let adj = graph.graphDistance.map((row) =>
        row.map((d) => (d == 1 ? 1.0 : 0.0)),
    );
    adj = tf.tensor2d(adj);
...
function initTableHeader(table, keys) {
    let headerRow = table.append("tr").attr("class", "tableHeader");
    headerRow.selectAll("th").data(keys).enter().append("th");

    headerRow.selectAll("th").text((d) => d);
}
...
function highlightBest(groupIndex, metricNames) {
    ...
    metricNames.forEach((name) => {
        ...
        d3.selectAll(`.${name}-group-${groupIndex}`).style(
            "font-weight",
            (d) => {
...
window.onload = function () {
    ...
    fnGroups.forEach((fnGroup, groupIndex) => {
        ...
        Promise.all(promises).then((graphs) => {
            ...
            let sizeEntry = tableRow
                .append("td")
                .text(
                    `|V|=${graphs[0].nodes.length}, |E|=${graphs[0].edges.length}`,
                );

            zip(fnGroup, graphs).forEach((fn_graph_pair) => {
...
function colorEdges(row) {
    ...
    let vmax =
        d3.max(data.slice(edge_count), (d) =>
            Math.abs(d.pdist - d.graph_dist),
        ) + 0.2;
    ...
    let sc = d3.scaleQuantile().domain([-vmax, vmax]).range(diverging_colors);

    edges.attr("stroke", (d) => sc(d.pdist - d.graph_dist));
}

function colorbar() {
    let sc = d3.scaleQuantile().domain([-1, 1]).range(diverging_colors);
    ...
    let g = svg.append("g").attr("class", "colorbar");
    ...
    let ax = d3
        ...
        .tickFormat((d) => {
            if (d == 0.5) {
                return "short";
            } else if (d == diverging_colors.length / 2) {
                //just right
                return "";
